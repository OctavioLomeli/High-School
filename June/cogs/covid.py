...
with open('discord_bot_covid_stats.csv', 'w') as myFile:
...
covid_stat = pd.read_csv('./discord_bot_covid_stats.csv')
covid_stat.set_index(covid_stat['State'], inplace=True)
covid_stat = covid_stat[['Cases', 'Deaths']]

# saves time for the functions cdm and ccm
df = pd.read_csv('./discord_bot_covid_stats.csv')
...
class Covid(commands.Cog):
    ...
    # Sends num of case for covid for a state
    @commands.command()
